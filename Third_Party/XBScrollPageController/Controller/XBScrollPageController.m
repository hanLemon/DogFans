...
- (void)setupDefaultProperties
{
    ...
    self.selectedTitleColor = [UIColor brownColor];
...
- (void)selectTagByIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.001f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self.tagCollectionView selectItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0] animated:NO scrollPosition:UICollectionViewScrollPositionCenteredHorizontally];
...
//-(void) setTagSelectedIndex:(NSInteger)selectedIndex{
//    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.001f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//        [self collectionView:self.tagCollectionView didSelectItemAtIndexPath:[NSIndexPath indexPathForItem:selectedIndex inSection:0]];
//    });
//}
